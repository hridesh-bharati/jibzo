...
const AdminProfile = () => {
  ...
  const handleDpUpdate = async () => {
    ...
    if (!cloudinaryPreset || !cloudinaryCloud)
      return toast.error("Cloudinary ENV missing");
    setUploading(true);
    try {
  ...
  const handleEmailUpdate = async () => {
    if (!currentUser || !newEmail || !emailPassword) {
  ...
  const handlePasswordUpdate = async () => {
    ...
    const isValid = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>]).{8,}$/.test(
      newPassword
    );
    if (!isValid) {
      return toast.error(
  ...
  if (loading) {
    return <p className="m-5 p-5 text-center">Loading profile...</p>;
  }
  if (!profileData) {
    return <p className="m-5 p-5 text-center">No profile found.</p>;
  }
  ...
  return (
    <div className="container p-0" style={{ maxWidth: 900 }}>
      ...
      <div className="row d-flex align-items-start justify-content-center mb-4 gap-3">
        ...
        <div className="col-md-6 text-start d-flex flex-column justify-content-center gap-2">
          ...
          >
            ...
            <button
              className="threeD-btn yellowBtn"
              style={{ width: "calc(50% - 0.25rem)" }}
      ...
      {isOwner && (
        ...
        >
          <div
            className="offcanvas-header border-bottom text-white"
                  ...
                  {socialLinks.list?.map((linkObj, idx) => (
                    <div key={idx} className="d-flex gap-2 mb-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          const newList = socialLinks.list.filter(
                            (_, i) => i !== idx
                          );
                          setSocialLinks({ list: newList });
                        }}
                ...
                <div className="mb-3">
                  ...
                  <div className="form-check form-switch">
                    <input
                      ...
                      type="checkbox"
                      id="profileLockSwitch"
                      checked={profileData.isLocked}
                      onChange={toggleLockProfile}
                    />
                    <label
                      className="form-check-label"
                      htmlFor="profileLockSwitch"
                    >
